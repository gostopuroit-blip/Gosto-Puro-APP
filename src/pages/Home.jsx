      ...
      <div className="mt-8">
        ...
        <div className="flex gap-3 overflow-x-auto hide-scrollbar px-5 pb-2">
          ...
          <Link
            key={occ.label}
            to={createPageUrl(`Recipes?occasion=${encodeURIComponent(occ.label)}`)}
            className="flex-shrink-0 flex flex-col items-center gap-2 bg-white dark:bg-[#1A2B20] rounded-2xl p-4 shadow-sm border border-gray-50 dark:border-[#2D4A38] w-[110px] hover:border-[#2D6A4F]/20 dark:hover:border-[#40916C]/30 hover:shadow-md transition-all duration-200 active:scale-[0.97]">

              <span className="text-2xl">{occ.icon}</span>
              <span className="text-[11px] font-semibold text-gray-700 dark:text-gray-300 text-center leading-tight">{occ.label}</span>
            </Link>
          )}
        </div>
      </div>
