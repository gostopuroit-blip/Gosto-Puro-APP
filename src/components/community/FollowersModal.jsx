...
export default function FollowersModal({ expertEmail, onClose, currentUser }) {
  ...
  useEffect(() => {
    const loadFollowers = async () => {
      try {
        ...
        toast.error("Errore nel caricamento dei follower");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleFollowToggle = async (followerEmail) => {
    if (!currentUser) {
    ...
    try {
      if (followingMap[followerEmail]) {
        ...
        if (record[0]) {
          ...
          setFollowingMap((prev) => ({ ...prev, [followerEmail]: false }));
        ...
        setFollowingMap((prev) => ({ ...prev, [followerEmail]: true }));
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end md:items-center justify-center p-4 pb-28 overflow-y-auto">
      <div className="bg-white dark:bg-[#1A1A1A] rounded-3xl w-full max-w-md shadow-xl flex flex-col my-auto max-h-[90vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 dark:border-[#2A2A2A] flex-shrink-0">
          <h2 className="text-lg font-bold text-gray-900 dark:text-white">Follower</h2>
          <button
            onClick={onClose}
        ...
        {loading ? (
          ...
          <div className="flex items-center justify-center py-12 flex-1">
            <p className="text-gray-400 text-sm">Nessun follower ancora</p>
          </div>
        ) : (
          <div className="space-y-2 overflow-y-auto flex-1 p-4">
            {followers.map((follower) => (
              <div key={follower.email} className="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-[#2A2A2A] transition">
                ...
                <Link
                  to={`/ExpertProfile?uid=${btoa(follower.email)}`}
