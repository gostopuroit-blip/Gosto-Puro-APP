...
export default function ProfileStatsCard({ userEmail, postCount, followerCount, followingCount, onPostClick, onFollowerClick, onFollowingClick }) {
  const [fetchedPosts, setFetchedPosts] = useState(postCount ?? null);

  useEffect(() => {
    if (postCount !== undefined) return;
    // Fetch by user_email (not created_by)
