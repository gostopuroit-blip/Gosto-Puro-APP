...
export default function AdminEngagement() {
  ...
  // PWA
  const pwaTotal = pwaClicks.length;
  const pwaUniqueUsers = new Set(pwaClicks.filter(e => e.user_email).map(e => e.user_email)).size;
  ...
  return (
    <div className="space-y-5">
      ...
      {/* 4. Session duration */}
      <Section title="⏱ Tempo medio in app" subtitle={`Basato su ${durations.length} sessioni`}>
        ...
        {durations.length === 0 && (
          <p className="text-[10px] text-gray-400 mt-2">Nessuna sessione completata registrata ancora.</p>
        )}
      </Section>
    </div>
  );
